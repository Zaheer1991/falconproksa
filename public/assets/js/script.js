...
(function ($) {
	...
	// 		// Ensure preloader stays visible for 5 seconds
	// 		$("#preloader").delay(2000).fadeOut(1000); // 5 second delay, 1 second fade out
	// 	});
	// };
	// loader();
    const loader = () => {
        $(window).on("load", function () {
            // Add an optional delay before fading out the preloader
            setTimeout(() => {
                $("#preloader").fadeOut(1000); // Fade out over 1 second
            }, 2000); // 2-second delay after the page has loaded
        });
    };
    loader();
